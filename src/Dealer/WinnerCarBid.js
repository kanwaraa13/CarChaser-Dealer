...
export const WinnerCarBid = () => {
  ...
  const [winningBidDetails, setWinningBidDetails] = useState([]);
  useEffect(() => {
    fetchData();
    bidfetchdata();
  }, []);
  ...
  const bidfetchdata = async () => {
    try {
      ...
      const winningBidResponse = await api.get(
        `/dealer/dealerwiningbiddetails/${Vehicle_Id}/${dealerId}`
      );
      setWinningBidDetails(winningBidResponse.data);

      const winnerDetails = winningBidResponse.data.WinnerDetails;
      if (winnerDetails && winnerDetails.length > 0) {
        // Assigning the bid amount to the bidAmount state variable
        const bidAmountValue = winnerDetails[0].Bid_Amount;
        setBidAmount(bidAmountValue); // Update the state with the bid amount
      } else {
      }
    } catch (error) {
    ...
    <section class="car-details">
      ...
      <div class="container mt-5">
        ...
        <div class="top-bid-price pt-3">
          ...
          <h6 className="pl-3">
            {winningBidDetails &&
            winningBidDetails.WinnerDetails &&
            winningBidDetails.WinnerDetails.length > 0
              ? winningBidDetails.WinnerDetails[0].Bid_Date.split(" ")[0]
              : "No winning date available"}
          </h6>
        </div>
        <div class="inspection-report-view pb-2">
          <h4>Winning Bid Price:</h4>
          <h6 className="pl-3">
            {bidAmount
              ? formatWithCommas(bidAmount)
              : "No winning bid amount available"}
          </h6>
        </div>
        <div class="row py-3">
          <div class="col-md-6">
